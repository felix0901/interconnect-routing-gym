...
iter_step = 6 # injection from 0.01 to 0.05
...
for i_episode in range(total_episodes):
	state = 0.01 # = Injection_rate as reset state env.reset()
	...
	for i in range(iter_step):
		next_state = (i+1)/100 # get next state
...
csv_file = "Inter_Connect_Networks/Tables/SARSA_"+str(iter_step)+'_'+str(total_episodes)+".csv"
...
plt.ylabel('Reward')
plt.savefig('SARSA_'+str(iter_step)+'_'+str(total_episodes)+'_ICN.png', bbox_inches='tight')